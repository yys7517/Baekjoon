...
//            System.out.println( result );
            int result = ParameterSearch( arr , budget );
            System.out.println( result );
        }

    }

    private static int ParameterSearch( int[] arr , long budget) {
        // 정렬
        Arrays.sort( arr );
        ...
        while ( start <= end ) {        // 최소값이 마지노선과 일치해지거나 높아지면 반복을 중단한다.
            ...
            for (int i = 0; i < arr.length; i++) {
                ...
                // 상한가만 더해준다.
                    ...
                    current += mid;
                // Why? 상한가를 더해주는 이유는.. 전체 요청 예산이 우리가 가지고 있는 budget보다 많기 때문에
                // 상한가를 더해줘야 나눠주지 못하는 지방 단체에게 공평하게 나눠줄 수 있다.
            }
            ...
            else if ( current > budget ){   // 가지고 있는 예산 budget보다 상한가를 통해 나눠주는 예산이 많을 때
        ...
        // 최대 예산을 구하는 것이므로.
        // 반복문이 종료되었을 때, end값 (마지노선 == 최댓값)을 반환해준다.
        return end;
    }
}
